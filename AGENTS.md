...
entities/                              entity YAML manifests (source of truth)
  account.yaml | opportunity.yaml | contact.yaml | email.yaml | transcript.yaml
src/
...
| `feat/dealbrain-schema` *(current HEAD)* | Same architecture, schema migrated to mirror dealbrain's live tables (no chunks, inline transcript body, contact entity, dealbrain-style opportunity columns including canonical Salesforce defaults flattened from EAV). |
