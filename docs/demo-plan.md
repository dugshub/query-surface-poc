...
clean, narratable CEO demo of a *lightweight semantic query layer for LLM agents*.
Make the web UI **visualize** cataloging, what's exposed, structured-query→deterministic-SQL,
the "% of deals mentioning competitor X" money shot, and live exposure toggles.
...
7. **Narrative section headers**: Catalog → Query→SQL → Govern, so a CEO can follow.
...
## Demo script (hand to the presenter)
...
3. **Money shot** — "What % of deals mention [Looker]?" → one click → "X of Y deals (Z%)".
   Exactly the CEO's example, on primitives (two counts), no metric layer needed.
...
version of Cube's `ratio` metric. (Closing-slide narrative — not built in this demo.)
