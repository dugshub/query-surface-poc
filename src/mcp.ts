...
const server = new McpServer(
  { name: 'query-surface', version: '0.1.0' },
  {
    ...
    instructions: [
      ...
      '  • person       ── the identity pillar: everyone in a communication is a person',
      '  • contact      ── belongs to ──> account, person   (a person promoted to a CRM stakeholder)',
      '  • opportunity  ── has many ──> communication        ── belongs to ──> account',
      '  • communication (type=email|meeting) ── belongs to ──> opportunity',
      '      ├─ email    (child of communication, type=email)',
      '      └─ meeting  (child of communication, type=meeting) ── has many ──> transcript',
      '  • communication ── has many ──> communication_participant ── belongs to ──> person',
      '      role: email→from/to/cc/bcc · meeting→host/invitee/attendee; meetings carry',
      '      response (RSVP) + attended — invited≠attended.',
      '',
      'So: who was on an email/meeting → communication.participants.person; which deals',
      'a person touched → person.participations.communication.opportunity; what was said on',
      'a call → transcript.transcript (transcript → meeting → participants for the attendees).',
    ].join('\n'),
  },
);
